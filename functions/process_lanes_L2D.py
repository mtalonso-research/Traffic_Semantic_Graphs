...
def load_yolo_annotations(episode_num, camera_name, frame_filename):
    ...
    possible_paths = [
        f'../data/processed_frames/L2D/Episode{episode_num:06d}/{camera_name}_Annotations/{base_name}.json'
    ]
...
def process_frame_with_vehicle_lane_classification(
    ...
    yolo_data = load_yolo_annotations(episode_num, camera_name, filename)
...
def process_episode_with_vehicle_lane_classification(
    ...
    """
    camera_frames_dir = f'../data/raw/L2D/frames/Episode{episode_num:06d}/observation.images.{camera_name}'
    episode_output_dir = os.path.join(output_base_dir, f'Episode{episode_num:06d}')
    ...
    for i, frame_file in enumerate(frame_files):
        ...
        result = process_frame_with_vehicle_lane_classification(
            frame_path, episode_num, camera_name, output_base_dir, verbose=verbose
        )
        if result:
...
# Batch entry point
# -----------------------------
def lane_processing(min_ep, max_ep=-1, output_base_dir='../data/processed_frames/L2D_lanes', verbose=False):
    """
